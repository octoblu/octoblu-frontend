...
RED.initializeEditor = function() {
    ...
    function validateNodeProperty(node,property,value) {
        ...
        if (valid && node._def.defaults[property].type && RED.nodes.getType(node._def.defaults[property].type) && !("validate" in node._def.defaults[property]) && node._def.defaults[property].required) {
        ...
        if(!valid){
            console.log('invalid node', node, 'property', property, 'value', value);
        }
        return valid;
    }
