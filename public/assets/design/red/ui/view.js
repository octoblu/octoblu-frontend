...
RED._initializeView = function() {
        ...
        if (mouse_mode != RED.state.JOINING) {
                   ...
                   //node.append("rect").attr("class", "node-gradient-bottom").attr("rx", 6).attr("ry", 6).attr("height",30).attr("stroke","none").attr("fill","url(#gradient-bottom)").style("pointer-events","none");
                    ...
                    if (d._def.icon) {
                        var icon = node.append("image")
                            .attr("xlink:href","/assets/design/icons/"+d._def.icon)
                    ...
                    node.append("image").attr("class","node_error hidden").attr("xlink:href","/assets/design/icons/node-error.png").attr("x",0).attr("y",-6).attr("width",10).attr("height",9);
                    node.append("image").attr("class","node_changed hidden").attr("xlink:href","/assets/design/icons/node-changed.png").attr("x",12).attr("y",-6).attr("width",10).attr("height",10);
            });
            ...
            node.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        if(d.pulse){
                            d.pulse = false;
                            thisNode.selectAll(".node").transition().duration(500)
    ...
    $('#btn-export-community').click(function() {showExportToCommunity();});
    ...
    function showExportToCommunity() {
        mouse_mode = RED.state.EXPORT;
        var nns = RED.nodes.createExportableNodeSet(moving_set);
        $("#dialog-form").html($("script[data-template-name='export-to-community']").html());
        $("#node-input-export-community").val(JSON.stringify(nns));
        $( "#dialog" ).dialog({
            modal: true,
            autoOpen: true,
            width: 500,
            ...
            buttons: [
                ...
                },
                {
                    text: "Share",
                    click: function() {
                        var form = $('#export-to-community-form'),
                            error_field = $('#node-export-community-error'),
                            content = '',
                            dialog = this,
                            api_url = RED.settings.shareURL,
                            title = $('#export-to-community-title').val(),
                            desc = $('#export-to-community-desc').val();
                        if(!api_url){
                            error_field.text('Invalid configuration.');
                            return;
                        }
                        // Validate
                        if(!title || title.length <= 6){
                            error_field.text('Please fill out the title. Title must be longer than 6 characters.');
                            return;
                        }else if(!desc || desc.length <= 3){
                            error_field.text('Please fill out the description. Description must be longer than 3 characters.');
                            return;
                        }
                        //Submit to community
                        content = "<custom-flow " +
                            ...
                            "</custom-flow>";
                        $.ajax({
                            timeout : 12 * 1000, // Twelve seconds (in ms)
                            type: "POST",
                            url: api_url + 'public/flows/add/',
                            dataType: "jsonp",
                            data : {
                                title : title,
                                content : content
                            ...
                            success: function(res){
                                if(res.error){
                                    error_field.text('Error: ' + res.error);
                                    return;
                                }else{
                                    $( dialog ).dialog( "close" );
                                    $( "#dialog" ).dialog({
                                        ...
                                        buttons: [
                                            ...
                                            },
                                            {
                                                ...
                                                click : function(){
                                                    $( this ).dialog( "close" );
                                                    window.location.href=api_url + res.url;
                                                }
                                            }
                                        ]
                                    });
                            ...
                            error : function(err){
                                console.log(err);
                                error_field.text("Error, please try again later.");
                            }
                        });
                    }
                }
            ...
            open: function(e) {
                var error_field = $('#node-export-community-error');
                if(!nns || !nns.length){
                    error_field.text("Please complete a flow first");
                    return;
                }
